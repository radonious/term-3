...
public:
    ...
    // 00000000 00000000 00000000 000000000000
    //    h         m       s         ms

    static char* ObjToBin(Time obj) {
        char* res = new char[8+8+8+12+4];
        sprintf(res,"%s %s %s %s\0",
                bitset<8>(obj.time.hour).to_string().c_str(),
                bitset<8>(obj.time.minute).to_string().c_str(),
                bitset<8>(obj.time.second).to_string().c_str(),
                bitset<12>(obj.time.msecond).to_string().c_str());
        return res;
    }

    static Time BinToObj (char* str) {
        Time res;
        vector <char*> array;
        ...
        res.time.hour = bitset<8>(array.at(0)).to_ulong();
        res.time.minute = bitset<8>(array.at(1)).to_ulong();
        res.time.second = bitset<8>(array.at(2)).to_ulong();
        res.time.msecond = bitset<12>(array.at(3)).to_ulong();
