...
public:
    ...
    //Setter
    void setDateYear(int year) {this->date.hour = year; }
    void setDateMonth(int month) {this->date.hour = month; }
    void setDateDay(int day) {this->date.hour = day; }
    ...
    static DT getSystemDate() {
        ...
        struct timeval utime{};
        gettimeofday(&utime, nullptr);
        t.msecond = utime.tv_usec / MSEC_PER_SEC;

        return t;
    }

    static DT addDate(DT t1, DT t2) {
        DT t3;
    ...
    static char* ObjToBin(Date obj) {
        char* res = new char[12*2+8*5+7];
        sprintf(res,"%s %s %s %s %s %s %s\0",
                bitset<12>(obj.date.year).to_string().c_str(),
                bitset<8>(obj.date.month).to_string().c_str(),
                bitset<8>(obj.date.day).to_string().c_str(),
                bitset<8>(obj.date.hour).to_string().c_str(),
                bitset<8>(obj.date.minute).to_string().c_str(),
                bitset<8>(obj.date.second).to_string().c_str(),
                bitset<12>(obj.date.msecond).to_string().c_str());
        return res;
    }

    static Date BinToObj (char* str) {
        Date res;
        vector <char*> array;
        ...
        res.date.year = bitset<12>(array.at(0)).to_ulong();
        res.date.month = bitset<8>(array.at(1)).to_ulong();
        res.date.day = bitset<8>(array.at(2)).to_ulong();
        res.date.hour = bitset<8>(array.at(3)).to_ulong();
        res.date.minute = bitset<8>(array.at(4)).to_ulong();
        res.date.second = bitset<8>(array.at(5)).to_ulong();
        res.date.msecond = bitset<12>(array.at(6)).to_ulong();

        return res;
    }

    char* ObjToBinFile(string filename) {
