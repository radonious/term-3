...
public:
    ...
    // Setters
    void setLogname(char* logname) {this->logname = logname; }
    ...
    friend ostream& operator<<(ostream& os,const LoggerFile& obj) {
        string line;
        fstream file(string("../Logs/") + obj.logname);
        if (file.is_open())
        {
            while (getline( file, line))
            {
    ...
    friend istream& operator>>(istream& os, LoggerFile& obj) {
        string line;
        fstream file(string("../Logs/") + obj.logname);
        if (file.is_open())
        {
    ...
    friend class LoggerFileIterator;
private:
    Time A;
    fstream file;
    char* logname;
